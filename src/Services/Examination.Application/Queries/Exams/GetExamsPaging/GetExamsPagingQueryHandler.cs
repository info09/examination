...
namespace Examination.Application.Queries.Exams.GetExamsPaging
{
    public class GetExamsPagingQueryHandler : IRequestHandler<GetExamsPagingQuery, ApiResult<PagedList<ExamDto>>>
    {
        ...
        public async Task<ApiResult<PagedList<ExamDto>>> Handle(GetExamsPagingQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = await _examRepository.GetExamsPagingAsync(request.CategoryId, request.SearchKeyword, request.PageIndex, request.PageSize);
            var items = _mapper.Map<List<ExamDto>>(result.Items);

            _logger.LogInformation("END: GetExamsPagingQueryHandler");
            var pagedResult = new PagedList<ExamDto>(items, result.MetaData.TotalCount, request.PageIndex, request.PageSize);
            return new ApiSuccessResult<PagedList<ExamDto>>(200, pagedResult);
        }
    }
}
